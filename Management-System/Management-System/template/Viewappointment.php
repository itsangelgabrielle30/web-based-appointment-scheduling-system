...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $DateTimeSchedule = $_POST['DateTimeSchedule'];
    $medicalIssue = $_POST['medicalissue'];

    $sql = 'UPDATE scheduling SET Fullname = ?, DateTimeSchedule = ?, MedicalIssue = ? WHERE ScheduleNo = ? AND AccountNo = ?';
    ...
    $stmt->bind_param('ssssi', $Fullname, $DateTimeSchedule, $medicalIssue, $ScheduleNo, $accountno);
    $stmt->execute();
    ...
    if ($stmt->affected_rows > 0) {
        ...
        $errorMessage = 'No rows updated or an error occurred: ' . $stmt->error;
    }
...
$sql = 'SELECT * FROM scheduling WHERE AccountNo = ? AND Status = "Awaiting Doctor Assignment"';
$stmt = $conn->prepare($sql);
                            ...
                            <?php if ($appointments): ?>
                                <?php foreach ($appointments as $index => $appointment): ?>
                                    <tr>
                                        ...
                                        <td>
                                        <?php 
                                            $status = !empty($appointment['Status']) ? htmlspecialchars($appointment['Status']) : 'Awaiting Doctor Assignment';
                                            echo $status;
                                            ?>
                                        ...
                                        <td class="actions">
                                                ...
                                                <a href="delete_appointment.php?id=<?php echo htmlspecialchars($appointment['ScheduleNo']); ?>" class="button-view delete" title="Delete" onclick="return confirm('Are you sure you want to delete this appointment?');"><i class="fas fa-trash-alt"></i></a>
                                            </td>
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="7" class="text-center">
                                    ...
                                    <p>
                                        Use the <i class="fas fa-pencil-alt footer-icon icon-edit"></i> <strong>Edit</strong> button to modify appointment details or the <i class="fas fa-trash-alt footer-icon icon-delete"></i> <strong>Delete</strong> button to remove an appointment from the system.
                                    </p>
                                </td>
                            </tr>
                        </tfoot>
